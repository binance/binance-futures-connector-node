...
class WebsocketBase extends APIBase {
  ...
  initConnect (url) {
    const ws = new WebSocketClient(url, { agent: this.httpsAgent })
   ...
   */
  pingServer () {
    if (!this.isConnected()) { this.logger.warn('Ping only can be sent when connection is ready.') } else {
  ...
  send (payload) {
    if (!this.isConnected()) { this.logger.warn('Send only can be sent when connection is ready.') } else {
