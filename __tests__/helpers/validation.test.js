...
describe('#validateRequiredParameters', () => {
  ...
  it.each([
    ...
    [
      { paramName1: {}, paramName2: null, paramName3: NaN },
      ['paramName1', 'paramName2', 'paramName3']
    ]
  ])(
    'should throw error when empty parameter exists',
    (paramObject, expectedParams) => {
      expect(() => {
...
describe('#hasOneOfParameters', () => {
  ...
  it.each([
    ...
    'should not throw error when at least one parameter is filled',
    (param, param2) => {
      expect(() => {
        hasOneOfParameters({ param, param2 })
      }).not.toThrow(MissingParameterError)
